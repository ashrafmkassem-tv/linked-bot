"""
FINAL LINKED BOT - UNIFIED MASTER 95 stocks - CBOE:SNDU + BOTH NOW - FIXED v3 - Railway Ready
...
L3_PATH_DASH = WATCH_DIR / "L3-sectors.json"

def load_l3_sectors():
    # Try both underscore and dash filenames, pick the one with more tickers
    candidates = []
    for p in [L3_PATH, L3_PATH_DASH]:
        if p.exists():
            try:
                data = json.loads(p.read_text(encoding='utf-8'))
                if isinstance(data, dict):
                    total = sum(len(v.get("tickers", v) if isinstance(v, dict) else v) for v in data.values())
                    candidates.append((total, data, p))
                elif isinstance(data, list):
                    candidates.append((len(data), {"General": data}, p))
            except:
                pass
    if candidates:
        # Pick file with most tickers
        candidates.sort(key=lambda x: x[0], reverse=True)
        best_count, best_data, best_path = candidates[0]
        print(f"L3 loaded from {best_path.name}: {best_count} tickers, {len(best_data)} sectors")
        # If file is list format (simple), keep it
        return best_data
    return load_json(L3_PATH, DEFAULT_L3)

L1_WATCH = load_json(L1_PATH, DEFAULT_L1)
L2_LEADERS = load_json(L2_PATH, DEFAULT_L2)
L3_SECTORS = load_l3_sectors()
...
def scan_L3():
    print(f"[{datetime.now()}] L3 Sectors Scan {len(L3_SECTORS)} - {sum(len(v.get('tickers', v) if isinstance(v, dict) else v) for v in L3_SECTORS.values())} total tickers")
    for sector_name, sector_data in L3_SECTORS.items():
        # Handle both dict {tickers: [...], keywords: [...]} and simple list [...] formats
        if isinstance(sector_data, dict):
            tickers = sector_data.get("tickers", [])
            keywords = sector_data.get("keywords", [sector_name])
        else:
            tickers = sector_data
            ...
            for n in get_hybrid(ticker)[:1]:
                ...
                if time.time() - n["datetime"] > 2*24*3600: continue
                low = (title + " " + n.get("summary","")).lower()
                has_sector = any(k.lower() in low for k in keywords) or ticker.lower() in low or sector_name.lower() in low
                has_gov = any(k.lower() in low for k in GOV_KEYWORDS)
...
def handle_telegram():
    ...
    tg(f"Linked Bot Started UNIFIED - L1 {len(L1_WATCH)} ({unique} unique) L2 {len(L2_LEADERS)} L3 {len(L3_SECTORS)} sectors 95 stocks CBOE:SNDU BOTH NOW FIXED - Webhook ready at /webhook/tradingview")
    while True:
        try:
                ...
                if lower.startswith("/start"):
                    ...
                    if layer=="L1":
                        ...
                        for k,v in L3_SECTORS.items():
                            tickers = v.get("tickers", v) if isinstance(v, dict) else v
                            m+=f"{k}: {', '.join(tickers)}\n"
                        tg(m[:4000], chat_id)
                    else:
...
async def webhook(req: Request):
    ...
    if ticker in L1_WATCH:
        ...
        for sec, sec_data in L3_SECTORS.items():
            sec_tickers = sec_data.get("tickers", sec_data) if isinstance(sec_data, dict) else sec_data
            # Handle both with and without exchange prefix
            sec_tickers_plain = [t.split(":")[-1] for t in sec_tickers]
            if ticker in sec_tickers or ticker in sec_tickers_plain or ticker.replace(".TO","") in sec_tickers_plain:
                layer_info = f"L3 {sec}"
                break
