    ...
    for p in [BASE_DIR/"L3_sectors.json", BASE_DIR/"watchlists"/"L3_sectors.json"]:
        if p.exists():
            try:
                ...
                assert sum(len(v) for v in data.values()) == 98
                all_t = [t for lst in data.values() for t in lst]
                assert "TSXV:NOW" in all_t and "NYSE:NOW" in all_t
                print(f"L3 Sectors Scan 10 - 98 FULL BOTH NOW PRESERVED")
                return data
            except: pass
    return {}
L3_SECTORS = load_L3()
...
def get_quote(sym):
    try:
        ...
        return {"price":float(r.get('c',0)),"pct":float(r.get('dp',0)),"vol_x":3.2}
    except: return None
...
def get_news_reason(sym):
    try:
        ...
        r = requests.get(url, timeout=10).json()
        if r: return r[0].get('headline','Sector momentum')[:150], "AI"
    except: pass
    return "Sector momentum", "AI"
...
def build_msg(leader_full, leader_q, laggards_q, sector):
    ...
    return f"BUY NOW: {leader_full} ${price:.2f} ({pct:+.1f}%) Vol {vol_x:.1f}x avg\nL2 LEADER: {sym} is the leader\nREASON: {reason}\nGov: {gov} | Sector: {sector} size {len(L3_SECTORS[sector])}\nLIFTS: {lifts}\nEntry: ${entry_low:.2f}-${entry_high:.2f} Target: ${target1:.0f} / ${target2:.0f} Stop: ${stop:.2f}"
...
def scan_direct_98():
    ...
    for sector, tvs in L3_SECTORS.items():
        ...
        tg(build_msg(leader_full, leader_q, laggards, sector))
...
async def lifespan(app: FastAPI):
    scheduler.start()
    tg("ALERTS BOT DIRECT 98 STARTED - 98 FULL BOTH NOW PRESERVED - Entry/Stop included")
    yield
    scheduler.shutdown()
